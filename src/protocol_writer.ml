...
module For_handshake = struct
  let handle_handshake_result ~step : _ Transport.Send_result.t -> _ = function
...
module Response = struct
  ...
  let send t id impl_menu_index ~data ~bin_writer_response =
  ...
  let handle_send_result
    ...
    rpc
    kind
    (result : _ Transport.Send_result.t)
    =
